...
namespace Toastify
{
	...
	static class Spotify
	{
		...
		public static void SendAction(SpotifyAction a)
		{
			...
			if (!Spotify.IsRunning())
			{
				if (!SettingsXml.Current.ChangeSpotifyVolumeOnly && SettingsXml.Current.VolumeControlsAlwaysAvailable)
				{
					switch (a)
					{
						...
						case SpotifyAction.VolumeUp:
							Win32.SendMessage(Process.GetProcessesByName("explorer")[0].MainWindowHandle, Win32.Constants.WM_APPCOMMAND, IntPtr.Zero, new IntPtr((long)a));
							break;
